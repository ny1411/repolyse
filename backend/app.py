...
# Set up Gemini API
GEMINI_API_KEY = "AIzaSyCM5KUEXu0Aed4LoxDzYs8D3NhQTv56j_4"
    ...
    def fetch_directory(path=""):
        try:
            ...
            for filename, file_obj in contents.items():
                ...
                if file_obj.type == "file" and filename.endswith((".py", ".js", ".c")):
                    if file_obj.decoded:
                        try:
...
def analyze_repo(repo_url):
    global progress
    output_dir = f"output_{hashlib.md5(repo_url.encode()).hexdigest()}"
